...
namespace IDE.Core.Routing.FastAStar
{
    ...
    public class SpatialAStar<TPathNode, TUserContext> where TPathNode : IPathNode//<TUserContext>
    {
        ...
        /// </summary>
        public LinkedList<TPathNode> Search(XPoint inStartNode, XPoint inEndNode, TUserContext inUserContext)
        {
            ...
            //    }
            //}
            var m_CameFrom = new PathNode[Width, Height];
            ...
            while (currentIteration < maxIterations && !m_OpenSet.IsEmpty)
            {
                currentIteration++;
                ...
                for (int i = 0; i < neighborNodes.Length; i++)
                {
                    ...
                    if (!y.IsWalkable(inUserContext) || !x.CanWalkTo(y))
                        continue;
        ...
        protected class PathNode : IPathNode/*<TUserContext>*/, IComparer<PathNode>, IIndexedObject
        {
            ...
            public bool CanWalkTo(IPathNode/*<TUserContext>*/ node)
            {
                return UserContext.CanWalkTo(node);
            }

            public void BuildArcs(IPathNode[] neighbors)
            {
                UserContext.BuildArcs(neighbors);
            }
