...
namespace IDE.Core.Spatial2D
{
    public partial class RTree<T>
    {
        ...
        private IList<T> DoSearch(Envelope boundingBox)
        {
        ...
        private IList<T> SearchNearestInternal(XPoint point, double maxDistance)
        {
            var intersections = new List<T>();
            var queue = new Queue<Node>();
            queue.Enqueue(Root);
            ...
            while (queue.Count != 0)
            {
                ...
                if (item.IsLeaf)
                {
                    foreach (T leafChildItem in item.Children.Cast<T>())
                    {
                        if (leafChildItem.Envelope.Distance(point) <= maxDistance)
                        {
                            intersections.Add(leafChildItem);
                        }
                    }
                ...
                else
                {
                    foreach (var child in item.Children.Cast<Node>())
                    {
                        if (child.Envelope.Distance(point) <= maxDistance)
                        {
                            queue.Enqueue(child);
                        }
                    }
