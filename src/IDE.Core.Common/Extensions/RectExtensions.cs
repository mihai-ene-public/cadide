...
namespace IDE.Core
{
    public static class RectExtensions
    {
        ...
        public static XPoint GetCenter(this XRect rect)
        {
            return new XPoint(rect.X + rect.Width / 2, rect.Y + rect.Height / 2);
        }
        ...
        public static bool Intersects(this XRect self, XRect rect)
        {
            return (self.IsEmpty || rect.IsEmpty)
                || (self.Width == Double.PositiveInfinity || self.Right >= rect.Left)
                && (rect.Width == Double.PositiveInfinity || rect.Right >= self.Left)
