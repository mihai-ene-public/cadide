...
{
    public class TextSingleLineBoardCanvasItemTests
    {
        [Theory]
        [InlineData(0, 0, "Text", 0, 0, 4, 1)]
        [InlineData(0, 0, "R1", 0, 0, 2, 1)]
        public void GetBoundingRectangle(double x, double y, string text,
           double expectedRectX, double expectedRectY, double expectedRectWidth, double expectedRectHeight)
        {
            var item = new TextSingleLineBoardCanvasItem
            {
                X = x,
                Y = y,
                Text = text,
