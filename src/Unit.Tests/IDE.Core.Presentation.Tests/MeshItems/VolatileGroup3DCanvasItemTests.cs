...
{
    public class VolatileGroup3DCanvasItemTests
    {
        [Theory]
        [InlineData(0, 0, 1, 1)]
        public void Translate(double x, double y, double dx, double dy)
        {
            var item = new VolatileGroup3DCanvasItem
            {
                X = x,
                Y = y,
            };
        ...
        [Theory]
        //translate only
        [InlineData(0, 0, 0, 1, 1, 0, 0)]//transofrm by is not implemented, so no transform is applied
