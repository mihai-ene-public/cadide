...
namespace IDE.Core.Common.Tests
{
    public class RTreeTests
    {
        ...
        public void InsertTestData()
        {
            ...
            Assert.Equal(GetEnvelopeForPoints(points), tree.Envelope);
        }

        private Envelope GetEnvelopeForPoints(IEnumerable<Point> sourcePoints)
        {
            var bounds = Envelope.EmptyBounds;
            foreach (var p in sourcePoints)
                bounds.Extend(p.Envelope);
        ...
        public void BasicRemoveTest()
        {
            ...
            //	tree.Envelope);
            var expected = GetEnvelopeForPoints(shouldFindPoints);
            var actual = tree.Envelope;
            Assert.Equal(expected, tree.Envelope);
        }
        ...
        public void TestSearchAfterInsert()
        {
            ...
            var firstSetEnvelope = GetEnvelopeForPoints(firstSet);
        ...
        public void TestSearchAfterInsertWithSplitRoot()
        {
            ...
            var extraPointsSetEnvelope = GetEnvelopeForPoints(extraPointsSet);
        ...
        public void TestSearchAfterBulkLoadWithSplitRoot()
        {
            ...
            var numExtraPoints = 5;
            var extraPointsSet = points.Skip(points.Length - numExtraPoints);
            // var extraPointsSetEnvelope = extraPointsSet.Aggregate(Envelope.EmptyBounds, (e, p) => e.Extend(p.Envelope));
            var extraPointsSetEnvelope = GetEnvelopeForPoints(extraPointsSet);
        ...
        public void MissingEnvelopeTestInsertIndividually()
        {
            ...
            //var expected = tree.Search().Where(p => p.Envelope.Intersects(envelope)).ToList();
            var expected = missingEnvelopeTestData.Where(p => p.Envelope.Intersects(envelope)).ToList();
            var actual = tree.Search(envelope);

            Assert.Equal(expected.Count, actual: actual.Count);
        }
